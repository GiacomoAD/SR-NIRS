...
class UIFunctions(MainWindow):
    ...
    def selectMenu(getStyle):
        select = getStyle + ("QPushButton { border-right: 7px solid rgb(255, 215, 0); }")
        return select

    ## ==> DESELECT
    def deselectMenu(getStyle):
        deselect = getStyle.replace("QPushButton { border-right: 7px solid rgb(255, 215, 0); }", "")
        return deselect
    ...
    def configMenu(self, configOn):
        if configOn:
            self.configOn = 0
            self.ui.stackedWidget_2.setCurrentWidget(self.ui.page_config_off)
            self.ui.button_config.setText('Open Config')

        else:
            self.configOn = 1
            self.ui.stackedWidget_2.setCurrentWidget(self.ui.page_config_on)
            self.ui.button_config.setText('Close Config')


        return configOn

    def sliderChange(self, value, label):
        if label == 1:
            self.ui.label_14.setText(str(value/100))
        elif label == 2:
            self.ui.label_13.setText(str(value/100))
